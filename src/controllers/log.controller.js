...
export const getAll = async (req, res) => {
  try {
    ...
    const logs = await logService.getAllLogs(req.user.employeeId, ip, tenantOf(req));
...
export const getById = async (req, res) => {
  try {
       let ip =
            req.headers["x-forwarded-for"]?.split(",")[0] || req.socket.remoteAddress;
    const log = await logService.getLogById(req.params.id, req.user.employeeId, ip, tenantOf(req));
    if (!log) {
